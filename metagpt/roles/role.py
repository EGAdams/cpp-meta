...
class RoleContext(BaseModel):
    ...
    def important_memory(self) -> list[Message]:
        """Obtain the information corresponding to the attention action"""
...
class Role:
    ...
    def _watch(self, actions: Iterable[Type[Action]]):
        """Monitor the corresponding behavior"""
    ...
    def set_env(self, env: 'Environment'):
        """Set the environment in which the character works, the character can speak to the environment, and can also receive environmental messages through observation"""
        self._rc.env = env
    ...
    async def _think(self) -> None:
        """Think about what to do and decide what to do next"""
        if len(self._actions) == 1:
            # If there is only one action, it can only do this
            self._set_state(0)
            return
